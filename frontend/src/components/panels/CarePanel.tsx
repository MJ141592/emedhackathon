...
export function CarePanel({ focus, notify, openUrgent }: { focus?: SuggestionKind; notify: (message: string) => void; openUrgent: () => void }) {
  ...
  const confirmContent = useMemo(() => {
    ...
    if (confirm === "prescription") return { title: "Ask the prescriber to review the rescue plan?", description: `This routes evidence to ${state.prescription.prescriber || "your named prescriber"}. It does not issue medicine. An authorised prescriber must decide, and Remi cannot select or change the dose.`, label: "Send request for prescriber review", action: () => { updatePrescription({ status: "requested" }); setConfirm(null); notify("Request sent for simulated prescriber review. No prescription has been issued."); } };
  ...
  return <div className="panel-stack care-stack">
    ...
    <section className={`feature-card ${focus === "prescription" ? "focus-ring" : ""}`} id="prescription-flow">
      ...
      <div className="guardrail"><Stethoscope /><p><b>Remi is not the prescriber.</b> {state.prescription.prescriber} owns the decision. {state.prescription.pharmacy} only receives an authorised prescription.</p></div>
      ...
      {state.prescription.status === "not-started" && <div className="inline-warning"><Stethoscope /><div><b>No clinician-owned rescue pathway is recorded</b><p>A clean profile does not inherit Matthew’s plan. This explicit demo import simulates a named clinician providing eligibility, review timing and an immutable schedule; Remi does not create it.</p><button className="btn" onClick={async () => notify(await importClinicalPlan() ? "Simulated clinician plan imported. Review and verify its exact schedule before use." : "The clinician-plan simulation could not be imported. Check the sync error and retry.")}>Simulate secure clinician-plan import</button></div></div>}
    ...
    <section className={`feature-card ${focus === "urgent" ? "focus-ring" : ""}`} id="safety-screen">
      ...
      <p>This checklist runs separately from Remi and any AI model.</p>
