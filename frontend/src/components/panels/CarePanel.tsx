...
function downloadSummary(contents: string, date: string) {
  ...
  anchor.href = href;
  anchor.download = `memed-clinician-summary-${date}.txt`;
  anchor.click();
...
export function CarePanel({ focus, notify, openUrgent }: { focus?: SuggestionKind; notify: (message: string) => void; openUrgent: () => void }) {
  ...
  return <div className="panel-stack care-stack">
    ...
    <section className={`feature-card ${focus === "team" ? "focus-ring" : ""}`} id="team-flow">
      ...
      {canPrepareFollowUp && <button className="btn" onClick={() => { const id = `MSG-${Date.now()}`; const subject = `Follow-up from ${state.profile.name || "MeMed patient"}`; updateTeamMessage({ id, subject, body: state.clinicianSummary, status: "draft", reply: undefined }); setMessageBody(state.clinicianSummary); notify("The prior sent message state was preserved and a new editable follow-up draft was prepared. Nothing was sent."); }}>Prepare next follow-up draft</button>}
    ...
    <section className={`feature-card ${focus === "prescription" ? "focus-ring" : ""}`} id="prescription-flow">
      ...
      {state.prescription.status === "not-started" && <div className="inline-warning"><Stethoscope /><div><b>No clinician-owned rescue pathway is recorded</b><p>A clean profile does not inherit Amara’s plan. This explicit demo import simulates a named clinician providing eligibility, review timing and an immutable schedule; Penny does not create it.</p><button className="btn" onClick={async () => notify(await importClinicalPlan() ? "Simulated clinician plan imported. Review and verify its exact schedule before use." : "The clinician-plan simulation could not be imported. Check the sync error and retry.")}>Simulate secure clinician-plan import</button></div></div>}
    ...
    <section className={`feature-card taper-card ${focus === "taper" ? "focus-ring" : ""}`} id="taper-flow">
      ...
      {state.taper.days.length > 0 && <details className="guidance"><summary>Review the exact imported schedule</summary><ol>{taperSteps.map((step, index) => { const nextStep = taperSteps[index + 1]; const endDay = (nextStep?.day ?? state.taper.days.length + 1) - 1; const end = state.taper.days.find((day) => day.day === endDay); return <li key={step.day}><b>{step.doseMg} mg</b> · days {step.day}–{endDay} · {step.date}{end && end.date !== step.date ? ` to ${end.date}` : ""}</li>; })}</ol><p>This is displayed exactly as imported from {state.taper.prescribedBy || "the recorded prescriber"}. MeMed cannot calculate, edit or accelerate it.</p></details>}
