...
export function PrivacyPanel({ notify, onClose }: { notify: (message: string) => void; onClose: () => void }) {
  ...
  return <div className="panel-stack privacy-panel">
    ...
    <section className="feature-card">
      <div className="section-heading"><div><p className="eyebrow">Remi’s permissioned context</p><h3>Choose which records Remi can use</h3></div><LockKeyhole /></div>
      ...
      <label className="toggle-row"><span><b>Earlier Remi conversation</b><small>Your prior messages, used only when a new question asks about them</small></span><input type="checkbox" checked={state.privacy.assistantConversationAccess} onChange={(e) => updatePrivacy({ assistantConversationAccess: e.target.checked })} /></label>
