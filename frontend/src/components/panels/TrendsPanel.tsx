...
export function TrendsPanel({ notify }: { notify: (message: string) => void }) {
  ...
  return <div className="panel-stack">
    ...
    <section className="feature-card">
      ...
      <p>MeMed only aligns timestamps in your included diary. It does not label foods as triggers or recommend exclusions from these episodes.</p>
    ...
    <section className="feature-card">
      ...
      <label htmlFor="clinician-summary">Only your approved words leave MeMed</label>
      <textarea id="clinician-summary" className="summary-editor" value={summary} onChange={(event) => setSummary(event.target.value)} />
      <div className="button-row"><button className="btn primary" onClick={() => { updateSummary(summary); notify("Summary draft saved locally. It has not been sent."); }}>Save draft</button><button className="btn" onClick={() => { regenerateSummary(); notify("Summary explicitly regenerated from currently included records. Review it before sharing."); }}>Regenerate from records</button><button className="btn" onClick={() => { updateSummary(summary); download("memed-clinician-summary.txt", summary, "text/plain"); notify("Summary exported to this device. Nothing was sent to a clinician."); }}><Download /> Export summary</button><button className="btn" onClick={() => download("memed-data-export.json", exportData())}><Download /> Export all evidence</button></div>
    </section>
