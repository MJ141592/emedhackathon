...
import { Camera, ChevronDown, Droplets, HeartPulse, LockKeyhole, Moon, Pill, Plus, Trash2, X } from "lucide-react";
...
function EntryRow({ entry, onEdit, onDelete, notify, deleteButtonRef }: { entry: JournalEntry; onEdit: () => void; onDelete: () => void; notify: (text: string) => void; deleteButtonRef?: (element: HTMLButtonElement | null) => void }) {
  ...
  return (
    <article id={`entry-${entry.id}`} className={`entry ${entry.kind === "Remi noticed" ? "penny-note" : ""} ${entry.flagged ? "flagged" : ""} ${entry.excluded ? "excluded" : ""}`} aria-busy={removingPhoto}>
      <div className="entry-top">
        <p className="meta">{entry.kind === "Remi noticed" ? entry.kind : `${entry.time} · ${entry.kind}`}{entry.flagged && <span className="pill flag">Flagged</span>}{entry.excluded && <span className="pill muted">Excluded</span>}</p>
...
function CaptureDialog({ kind, onClose, notify }: { kind: CaptureKind; onClose: () => void; notify: (text: string) => void }) {
  ...
  const analyzePhoto = async () => {
    ...
    if (!state.privacy.assistantJournalAccess) return notify("Remi’s Journal and photos access is off. The image remains in your correctable record and will not be sent for model analysis.");
  ...
  const submit = async () => {
    ...
    if (!saved) return notify("That entry was not saved. Keep Gutsy open and try again when the API is available.");
    notify(kind.includes("photo") || kind === "meal" ? "Saved securely. You can close Gutsy — background observations stay reviewable, and no calories are shown." : "Saved to your journal against your own baseline.");
    onClose();
  };
    ...
    <div className="modal-layer">
      <section ref={dialogRef} className="capture-dialog" role="dialog" aria-modal="true" aria-labelledby="capture-title" aria-busy={saving}>
        ...
        {(kind === "meal" || kind === "toilet-photo") && <>
          ...
          <p className="privacy-note">Gutsy never calculates calories, macros, meal scores or moral judgements.</p>
        </>}
...
function EditDialog({ entry, onClose }: { entry: JournalEntry; onClose: () => void }) {
  ...
  return <div className="modal-layer"><section ref={dialogRef} className="capture-dialog" role="dialog" aria-modal="true" aria-labelledby="edit-title"><button ref={closeRef} className="icon-btn close-corner" onClick={onClose} aria-label="Close editor"><X /></button><p className="eyebrow">Correct the source record</p><h2 id="edit-title">Edit journal entry</h2><label>Entry type<select value={kind} onChange={(e) => setKind(e.target.value as EntryKind)}>{["BOWEL MOVEMENT","MEAL","PAIN","FATIGUE","WELLBEING","LIFE EVENT","MEDICATION","FROM YOUR WATCH","Remi noticed"].map((value) => <option key={value}>{value}</option>)}</select></label><label>What should the record say?<textarea value={body} onChange={(e) => setBody(e.target.value)} /><span className="field-help">Explicit details such as “Bristol type 4, no blood and no urgency” also correct the structured evidence used by patterns.</span></label>{entry.photo && <label>Correct the image-derived observation<textarea value={derived} onChange={(e) => setDerived(e.target.value)} /><span className="field-help">This remains an observation, not a diagnosis or trigger.</span></label>}<div className="button-row end"><button className="btn" onClick={onClose}>Cancel</button><button className="btn primary" onClick={() => { if (!body.trim()) return; updateEntry(entry.id, { kind, body: body.trim(), ...(entry.photo ? { photo: { ...entry.photo, derivedObservation: derived.trim() } } : {}) }); onClose(); }}>Save correction</button></div></section></div>;
}

export function JournalPanel({ notify, onOpenCare, onOpenSafetyCheck = () => onOpenCare("taper"), trackingEnabled, onClose }: { notify: (text: string) => void; onOpenCare: (focus?: "test" | "team" | "taper") => void; onOpenSafetyCheck?: () => void; trackingEnabled: boolean; onClose?: () => void }) {
  ...
  const quickPhoto = async (event: ChangeEvent<HTMLInputElement>, purpose: "meal" | "toilet") => {
    ...
    try {
      ...
      notify(saved
        ? `${purpose === "meal" ? "Meal" : "Toilet"} photo saved with your ${state.privacy.photoRetentionDays}-day retention setting. You can close Gutsy or edit it later; no calories or clinical action were inferred.`
        : "That photo was not saved. Keep Gutsy open and try again when the API is available.");
    } catch {
      notify("That image could not be read. Choose another file.");
    } finally {
      setQuickAction(null);
  ...
  const confirmEntryDeletion = async () => {
    ...
    notify("Entry deleted and removed from Remi’s evidence.");
  };

  return (
    <aside className="journal" aria-labelledby="journal-heading">
      <div className="journalhead"><div><h2 ref={journalHeadingRef} id="journal-heading" tabIndex={-1}>Journal</h2><p className="sub">What Remi reads — every entry is correctable.</p></div><div className="journalhead-actions"><button className="icon-btn add" disabled={!trackingEnabled || Boolean(quickAction)} onClick={() => setCapture("bowel")} aria-label="Add journal entry"><Plus /></button>{onClose && <button className="icon-btn" onClick={onClose} aria-label="Close journal"><X /></button>}</div></div>
      ...
      {!trackingEnabled && <p className="quick-note"><LockKeyhole aria-hidden="true" /> Capture is paused. Existing entries stay correctable and deletable.</p>}
      ...
      <div className="fast-photo-actions" aria-label="Fire-and-forget photo capture">
        ...
        <button className="text-btn" disabled={!trackingEnabled || Boolean(quickAction)} onClick={() => quickMealRef.current?.click()}><Camera aria-hidden="true" /> {quickAction === "meal" ? "Saving meal photo…" : "Meal camera"}</button>
        <button className="text-btn" disabled={!trackingEnabled || Boolean(quickAction)} onClick={() => state.privacy.toiletPhotoConsent ? quickToiletRef.current?.click() : setCapture("toilet-photo")}><Camera aria-hidden="true" /> {quickAction === "toilet" ? "Saving toilet photo…" : `Toilet camera${!state.privacy.toiletPhotoConsent ? " · consent first" : ""}`}</button>
      </div>
      ...
      <fieldset className="one-tap-checkin" disabled={!trackingEnabled || Boolean(quickAction)}><legend>Compared with your usual</legend><div className="segmented">{(["better", "same", "worse"] as const).map((value) => <button key={value} onClick={() => oneTapCheckIn(value)}>{value}</button>)}</div></fieldset>
      ...
      {groups.length === 0 && <div className="empty-state"><b>No journal entries</b><span>Add one with a quick action or by talking to Remi.</span></div>}
